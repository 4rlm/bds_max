class DashboardService
    ...
    def mega_dash
        dash(Core)
        list_getter(Core, [:bds_status, :staff_indexer_status, :location_indexer_status, :staffer_status, :geo_status, :who_status, :sfdc_tier, :sfdc_sales_person, :sfdc_type, :acct_source, :url_match_status, :acct_merge_sts])
        puts "\n\n#{'-'*50}\n\n"
        dash(InHostPo)
        ...
        puts "\n\n#{'-'*50}\n\n"
        dash(Staffer)
        list_getter(Staffer, [:staffer_status, :cont_status, :cont_source, :sfdc_type, :template])
        puts "\n\n#{'-'*50}\n\n"
        dash(Who)
